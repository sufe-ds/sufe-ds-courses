...
private:
  ...
  bool isFullBinTreeHelper(BinTreeNode<T> *node) {
    if (node == nullptr)
      return true;
    if ((node->left == nullptr) != (node->right == nullptr))
      return false;
    return isFullBinTreeHelper(node->left) && isFullBinTreeHelper(node->right);
  }
...
public:
  ...
  bool isFullBinTree() { return isFullBinTreeHelper(root); }
};

int main() {
  auto fullBinTree = BinTree<int>(-1, {1, 2, 3, 4, 5, 6, 7});
  auto notFullBinTree = BinTree<int>(-1, {1, 2, 3, 4, -1, 6, 7});
  std::cout << "Full:     " << fullBinTree.isFullBinTree() << std::endl;
  std::cout << "Not full: " << notFullBinTree.isFullBinTree() << std::endl;
  return 0;
}
